...
export class DataStreamV1<T> {
     ...
     * @param data
     */
    static from<V>(data: V[] | Set<V>): DataStreamV1<V> {
        if (Array.isArray(data)) {
            return new DataStreamV1(data);
        } else if (data instanceof Set) {
            return new DataStreamV1(Array.from(data));
        } else {
     ...
     * @param iterator
     */
    static fromIterable<T>(iterator: IterableIterator<T>): DataStreamV1<T> {
        return new DataStreamV1(iterator)
    }
     ...
     * @param data
     */
    static of<V>(...data: V[]): DataStreamV1<V> {
        return new DataStreamV1(data);
    }
     ...
     * @param data
     */
    static fromMap<K, V>(data: Map<K, V>): DataStreamV1<Kv<K, V>> {
        // entry 就是 T
        const mapEntryArray: Kv<K, V>[] = KvUtils.generateKvArrayByMap(data)
        return new DataStreamV1(mapEntryArray);
    }

    /**
     * 创建一个新的空白流
     */
    static createNew<T>(): DataStreamV1<T> {
        return new DataStreamV1([]);
    }
     ...
     * @param stream
     */
    mergeStream(stream: DataStreamV1<T>): DataStreamV1<T> {
        const self = this
     ...
     */
    distinct<U>(distinctFunction: DistinctFunction<T, U> | null = null): DataStreamV1<T> {
        ...
        return DataStreamV1.from(outputArray);
    };
     ...
     */
    sortByComparator(comparator: (a: T, b: T) => number): DataStreamV1<T> {
        const sortedArray = Array.from(this.dataIterator).sort(comparator);
        return new DataStreamV1(sortedArray);
    }
    ...
    sortByWeight(objToWeightNumberFunc: ConvertFunction<T, number> = null,
        ...
        return new DataStreamV1(newArray);
    }
     ...
     * @param objToWeightNumberFunc 将对象转为权重 weight 的函数
     */
    sortByWeightReverse(objToWeightNumberFunc: ConvertFunction<T, number> = null): DataStreamV1<T> {
     ...
     * @param mapFunc 映射函数
     */
    map<U>(mapFunc: ConvertFunction<T, U>): DataStreamV1<U> {
     ...
     */
    handle(handlerFunction: (item: T | null) => void): DataStreamV1<T> {
        for (const item of this.dataIterator) {
            // 处理每一个对象
    ...
    // 语法糖
    forEach(handlerFunction: (item: T) => void): DataStreamV1<T> {
     ...
     */
    flatMap<U>(flatMapFunc: ConvertFunction<T, Iterable<U>>): DataStreamV1<U> {
        ...
        return DataStreamV1.from(flatMapArray)
    }

    /**
     * 通过转为 DataStreamV1 来进行平铺 DataStreamV1 内部的所有元素
     * @param flatMapFunc 平铺成 DataStreamV1 的函数
     */
    flatMapByDataStreamV1<U>(flatMapFunc: ConvertFunction<T, DataStreamV1<U>>): DataStreamV1<U> {
        const flatMapArray = new Array<U>()
        for (const sublist of this.dataIterator) {
            const subArray = flatMapFunc(sublist);
            if (!subArray) {
     ...
     */
    filter(filterFunction: PredicateFunc<T>): DataStreamV1<T> {
        ...
        return DataStreamV1.from(filterArray);
    }

    /**
     * 过滤非空
     */
    filterNotNull(): DataStreamV1<T> {
    ...
    // 限制数量 | 取前 count 个元素
    limitInCount(count: number): DataStreamV1<T> {
        ...
        return DataStreamV1.from(limitArray);
    }
     ...
     * @param otherArray
     */
    mergeData(otherArray: T[] | Set<T>): DataStreamV1<T> {
        const otherStream = DataStreamV1.from(otherArray);
